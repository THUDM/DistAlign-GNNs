...
class SIGN(BaseModel):
    ...
    def __init__(self,
        ...
        self.prelu = nn.PReLU()
        self.inception_mlp = nn.ModuleList()
        for hop in range(nhop + 1):
            self.inception_mlp.append(MLP(in_feats, hidden_size, hidden_size, n_layers, dropout))
        self.project = MLP((nhop + 1) * hidden_size, out_feats, hidden_size, n_layers, dropout)
        self.nhop = nhop
...
#         print("Preprocessing Done...")
    ...
    def forward(self, graph):
        ...
        x = self.cache_x
        for feat, mlp in zip(x, self.inception_mlp):
            hidden.append(mlp(feat))
