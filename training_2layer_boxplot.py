...
def get_model(model_name, dataset, args):
    if model_name == "gcn":
        ...
        from custom_models.sign import SIGN
        
        model = SIGN(in_feats=dataset.num_features,
                 out_feats=dataset.num_classes,
                 hidden_size=args.hidden_size,
                 nhop=args.num_layers,
                 n_layers=args.num_layers,
                 dropout=args.dropout,
                 adj_norm=["sym"],
                 diffusion="sgc")
    elif model_name == "gamlp":
        from custom_models.gamlp import GAMLP
        
        model = GAMLP(in_feats=dataset.num_features,
                 out_feats=dataset.num_classes,
                 hidden_size=args.hidden_size,
                 nhop=args.num_layers,
                 n_layers=args.num_layers,
        ...
        from custom_models.sage import GraphSAGE
        
        model = GraphSAGE(in_feats=dataset.num_features,
                 out_feats=dataset.num_classes,
...
if __name__ == '__main__':
    ...
    checkpoint_dir = "./save_models_bp/{}/{}".format(args.model_name, args.dataset_name)
    ...
    figure_dir = "./save_figures_2layer_bp/{}/{}/{}{}_{}_{}".format(args.model_name, args.dataset_name, "res_" if args.residual else "", args.hidden_size, args.num_layers, args.dropout)
    if not os.path.exists(figure_dir):
        os.makedirs(figure_dir)
    log_dir = "./log_2layer_bp/{}/{}".format(args.model_name, args.dataset_name)
    ...
    acc_dict['o'] = plot_acc(dataset, models_dict, args)
    # Post Augmentation Test
    for prop in [False]:
        if args.model_name == 'gamlp' or args.model_name == 'sign':
            for loc in ['n', 'c']:
                acc_dict[loc] = plot_postaug(dataset, models_dict, args, n_post=args.n_post, loc=loc, prop=prop)
        else:
            for loc in ['n', 'a', 'b', 'c']:
                acc_dict[loc] = plot_postaug(dataset, models_dict, args, n_post=args.n_post, loc=loc, prop=prop)
#                 print(len(acc_dict[loc]))

    data = list(acc_dict.values())
    import pickle
    with open("boxplot_data_{}".format(args.model_name), "wb") as fp:   #Pickling
        pickle.dump(data, fp)
        
    loc_to_labels = {'n': 'NP', 'a': 'PA', 'b': 'PB', 'c': 'PC', 'o': 'O'}
    fix, ax = plt.subplots()
    ax.boxplot(data)
    plt.yticks(fontsize=30, fontweight='bold')
    plt.xticks(ticks=np.arange(len(acc_dict.keys())) + 1, labels=[loc_to_labels[loc] for loc in acc_dict.keys()], fontsize=36, fontweight='bold')
    plt.ylabel("Accuracy", fontsize=36, fontweight='bold')
    plt.title('{}, {}'.format(args.model_name.upper(), args.dataset_name), fontsize=42, fontweight='bold')
